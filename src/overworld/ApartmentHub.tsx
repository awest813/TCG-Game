...
import { Engine, Scene, ArcRotateCamera, Vector3, HemisphericLight, MeshBuilder, StandardMaterial, Color3, GlowLayer } from '@babylonjs/core';
...
export const ApartmentHub: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Aesthetic: Deep moody blues for the apartment
    ...
    const termMat = new StandardMaterial("termMat", scene);
    termMat.emissiveColor = Color3.FromHexString("#00f2ff");
    terminal.material = termMat;

    // Window (View changes by time of day)
    ...
    scene.onPointerDown = (evt, pickResult) => {
        if (pickResult.hit && pickResult.pickedMesh) {
            ...
            if (name === "terminal") setScene('DECK_EDITOR');
            if (name === "bed") {
                if (confirm("Rest until next time block?")) {
                    advanceTime();
                }
            }
        }
    ...
    <div className="apartment-container fade-in" style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="ui-overlay" style={{ 
            ...
            <div className="glass-morphism" style={{ padding: '15px 30px', borderLeft: '5px solid var(--accent-magenta)' }}>
                <div style={{ color: 'white', fontWeight: 'bold', fontSize: '1.2rem', letterSpacing: '2px' }}>
                    {state.timeOfDay}
                </div>
                <div style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', textTransform: 'uppercase' }}>
                    Sunset Heights // Sector 7
                </div>
            </div>
        ...
        <div style={{ position: 'absolute', right: '40px', top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <div className="sidebar-icon" onClick={() => setScene('DECK_EDITOR')}>📇</div>
        ...
        <div className="bottom-nav glass-panel" style={{ 
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
                <button className="neo-button" onClick={() => { saveGame(); alert("Progress Encrypted & Saved."); }}>
                    SAVE STATE
                </button>
                <button className="neo-button" onClick={() => setScene('DECK_EDITOR')}>
                    TERMINAL
                </button>
            </div>

            <button className="neo-button primary" onClick={() => setScene('TRANSIT')} style={{ padding: '15px 40px' }}>
                GO TO METRO STATION
            </button>
        </div>
        ...
        {/* Wake Up Modal */}
        {showWakeUp && (
            <div className="wake-up-overlay" style={{
                position: 'fixed',
                inset: 0,
                background: 'black',
                display: 'flex',
            ...
            }} onAnimationEnd={() => setShowWakeUp(false)}>
