    ...
    return (
        <div className="profile-scene fade-in" style={{ 
            ...
            <div style={{ width: '100%', maxWidth: '1000px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '60px', zIndex: 10 }}>
                <div>
                    <h1 className="glow-text" style={{ fontSize: '1rem', letterSpacing: '10px', opacity: 0.5 }}>DUELIST_PASSPORT_ACTIVE</h1>
                    <h2 style={{ fontSize: '3.5rem', fontWeight: 900, margin: 0 }}>{profile.name.toUpperCase()}</h2>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '40px', width: '100%', maxWidth: '1000px', zIndex: 10 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                    <div className="glass-panel" style={{ padding: '0', overflow: 'hidden', height: '350px', position: 'relative' }}>
                        ...
                        <div style={{ textAlign: 'center', paddingTop: '60px' }}>
                            <div style={{ fontSize: '8rem', opacity: 0.1 }}>⚔️</div>
                            <div style={{ fontSize: '0.9rem', color: 'var(--accent-cyan)', letterSpacing: '5px', marginTop: '20px' }}>PRO-RANK {profile.level}</div>
                            <div style={{ width: '80%', height: '6px', background: 'rgba(255,255,255,0.1)', margin: '20px auto', borderRadius: '3px', position: 'relative' }}>
                                <div style={{ position: 'absolute', top: 0, left: 0, height: '100%', width: `${(profile.xp % 1000) / 10}%`, background: 'var(--accent-cyan)', boxShadow: '0 0 15px var(--accent-cyan)' }} />
                            </div>
                            <div style={{ fontSize: '0.6rem', opacity: 0.5 }}>SYNC RESONANCE: {(profile.xp % 1000) / 10}%</div>
                        </div>
                    </div>

                    <div className="glass-panel" style={{ padding: '30px' }}>
                        <h3 style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', letterSpacing: '2px', marginBottom: '20px' }}>GRAND_MEDALS_EARNED</h3>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px' }}>
                            {profile.badges.length > 0 ? profile.badges.map(b => (
                                ...
                                [1,2,3,4,5,6,7,8].map(i => <div key={i} style={{ width: '100%', aspectRatio: '1', background: 'rgba(255,255,255,0.02)', borderRadius: '5px', border: '1px dashed rgba(255,255,255,0.1)' }} />)
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                    <div className="glass-panel" style={{ padding: '40px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
                        <StatItem label="TOTAL WINS" value={profile.stats.wins} />
                        <StatItem label="DEFEATS" value={profile.stats.losses} />
                        <StatItem label="BIO-SYNC" value={profile.stats.cardsCollected} />
                        <StatItem label="TITLES" value={profile.stats.tournamentsWon} />
                    </div>

                    <div className="glass-panel" style={{ padding: '40px', flex: 1 }}>
                         <h3 style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', letterSpacing: '2px', marginBottom: '30px' }}>DUELIST_COLLECTION</h3>
