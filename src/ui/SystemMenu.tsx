...
export const SystemMenu: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <div className="system-overlay fade-in" onClick={onClose}>
      <div className="glass-panel sonsotyo-modal" onClick={(event) => event.stopPropagation()} style={{ maxWidth: '960px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', alignItems: 'flex-start', marginBottom: '24px' }}>
          <div>
            <div className="sonsotyo-kicker">v2.1.0 System Config</div>
            <h2 className="sonsotyo-title" style={{ fontSize: 'clamp(2.2rem, 5vw, 3.8rem)', marginTop: '8px' }}>Options</h2>
          </div>
          <button onClick={onClose} className="neo-button" aria-label="Close settings">Close</button>
        </div>

        <div className="sonsotyo-grid cards" style={{ gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
          <SettingCard label={`Master Volume ${volume}%`}>
            <input type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(parseInt(event.target.value, 10))} style={{ width: '100%' }} />
          </SettingCard>

          <SettingCard label="Combat Flow">
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {['NORMAL', 'TURBO'].map((speed) => (
                <button key={speed} onClick={() => setAnimSpeed(speed)} className={`neo-button ${animSpeed === speed ? 'primary' : ''}`}>{speed}</button>
              ))}
            </div>
          </SettingCard>

          <SettingCard label="Display Mode">
            <button className="neo-button" onClick={handleFullscreen} style={{ width: '100%' }}>Toggle Fullscreen</button>
          </SettingCard>

          <SettingCard label="Screen Shake">
            <button onClick={() => setScreenShake(!screenShake)} className={`neo-button ${screenShake ? 'primary' : ''}`}>{screenShake ? 'Enabled' : 'Disabled'}</button>
          </SettingCard>

          <SettingCard label="Presentation Sync" wide>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '8px' }}>
              {(['LOW', 'MEDIUM', 'HIGH', 'ULTRA'] as const).map((tier) => (
                <button key={tier} onClick={() => updateGameState({ visuals: { presentationTier: tier } })} className={`neo-button ${state.visuals.presentationTier === tier ? 'primary' : ''}`} style={{ fontSize: '0.65rem' }}>
                  {tier}
                </button>
              ))}
            </div>
          ...
          <SettingCard label="Auto-Sync">
            <button onClick={() => setAutoSave(!autoSave)} className={`neo-button ${autoSave ? 'primary' : ''}`}>{autoSave ? 'Enabled' : 'Disabled'}</button>
          </SettingCard>
        ...
        <div style={{ marginTop: '24px', display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <button className="neo-button primary" onClick={handleSave}>Sync Data</button>
          <button className="neo-button" onClick={handleQuit}>Return To Menu</button>
        </div>
      </div>
    </div>
  );
